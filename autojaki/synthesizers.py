...
@dc.dataclass
class Midi:
    midi_channel: int = dc.field(repr=False)
    note_number: int
    velocity: int
...
class FakeMidiSynthesizer(Synthesizer):
    ...
    def play_note(self, note: Note, follows_double: bool = False) -> Generator[Midi, None, None]:
        if note.is_single:
            ...
            yield Midi(midi_channel=self.midi_channel, note_number=self.note_number, velocity=velocity)
        elif note.is_double:
            yield Midi(midi_channel=self.midi_channel, note_number=self.note_number, velocity=self.velocities.normal)
            yield Midi(midi_channel=self.midi_channel, note_number=self.note_number, velocity=self.velocities.soft)
